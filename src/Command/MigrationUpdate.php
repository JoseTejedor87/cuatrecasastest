...
class MigrationUpdate extends Command
{
    protected static $defaultName = 'app:migrationUpdate';
    private $fromThisDate;
    ...
    public function __construct(ContainerInterface $container, LoggerInterface $logger)
    {
        ...
        $this->logger = $logger;
        $this->fromThisDate = '2020-08-18 00:00:00';
        
        // reemplazar por los ID validos que al superarlos se deba crear el json
        $this->ids = [  'eventos' => 0,
                        'areas_practicas' => 0,
                        'eventos_ponente' => 0,
                        'premios' => 0,
    ...
    protected function configure()
    {
        $this
            ...
            ->setHelp("Este comando sirve para Actualizar la  de la BBDD Web cuatrecasas Cms a web cuatrecasas de la nueva web")
            // the full command description shown when running the command with
            ->setDescription('Este comando sirve para Actualizar de la BBDD Web cuatrecasas Cms a web cuatrecasas de la nueva web')
            // Set options
            ->setDefinition(
    ...
    {
        $output->writeln([
            'Empezando el UPDATE de la migración',// A line
            '======================',// Another line
        ]);
        ...
        if($table=="all"){
            $output->writeln("Se van a exportar todos los registros las tablas que superen la fecha: {$fromThisDate}");
            $this->Abogados($conn);
            $this->Activity($conn);
    ...
    public function Abogados($conn){
        $query = "SELECT a.*, o.* FROM abogado a inner JOIN abogado_desc o ON o.id_abogado = a.id  
                    WHERE a.fecha_modificacion > {$fromThisDate} order by a.id";
        $stmt = $conn->prepare($query);
        $stmt->execute();
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/abogados.json', json_encode($results));
    ...
    public function Eventos($conn,$output){
        $query = "SELECT [id] ,[lang] ,[titulo] ,[resumen] ,[fecha_inicio] ,[fecha_final] ,[url_pdf] ,[email] ,[lugar] ,[mapa] ,[rss] ,[twitter] ,[facebook] ,[url_friend] ,[tags] ,[status] ,[ciudad] ,[principal] ,[image] ,[url_video] ,[url_inscripcion] ,[descripcion_lugar] ,[ubicacion_lugar] ,[contacto] ,[telefono] ,[programa] ,[Notificado] ,[fechaNotificacion] ,[destacada] ,[image_slider] ,[tipo] ,[visible] ,[aforo] ,[image_mail] ,[restricted], [status],[visio_esp] ,[visio_por] ,[visio_eng], [visio_chi], [url_friend]
          FROM eventos  WHERE id > {$this->ids['eventos']} order by id";
                $stmt = $conn->prepare($query);
        $stmt->execute();
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/eventos.json', json_encode($results));
    ...
    public function Activity($conn){
        $query = "SELECT [id] ,[lang] ,[titulo] ,[descripcion] ,[experiencia] ,[tags] ,[url_friend] ,[id_area] ,[url_image] ,[quote] ,[spractica] ,[sap] ,[visio_esp] ,[visio_por] ,[visio_eng], [visio_chi], [url_friend]
         FROM areas_practicas WHERE id > {$this->ids['areas_practicas']}";
        $stmt = $conn->prepare($query);
        $stmt->execute();
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/areas_practicas.json', json_encode($results));
    ...
    public function relatedActivities($conn){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/areas_relacionades.json', json_encode($results));
    ...
    public function EventosArea($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/eventosArea.json', json_encode($results));
    ...
    public function EventosPonente($conn,$output){
        $query = "SELECT [id] ,[id_evento] ,[lang] ,[nombre] ,[apellidos] ,[cargo] ,[telefono] ,[email] ,[descripcion] ,[image] ,[link] ,[id_abogado] ,[empresa] ,[orden] ,[hash]  
            FROM eventos_ponente  WHERE id > {$this->ids['eventos_ponente']}";        $stmt = $conn->prepare($query);
        $stmt->execute();
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/eventosPonente.json', json_encode($results));
    ...
    public function AbogadoArea($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/abogadoArea.json', json_encode($results));
    ...
    public function AreasQuotes($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/areasQuotes.json', json_encode($results));
    ...
    public function Premios($conn,$output){
        $query = "SELECT [id] ,[lang] ,[title] ,[otorgado] ,[fecha]  ,[desc_award] ,[desc_award_firma] ,[desc_award_indiv] ,[tags] ,[url_image] ,[url_friend] ,[destacado] ,[posicion] ,[rss] ,[facebook] ,[twitter] ,[visio_esp]  ,[visio_por] ,[visio_eng] ,[status] ,[otorgado_a] ,[orden] ,[visio_chi] 
        FROM premios WHERE id > {$this->ids['premios']}";  
        $stmt = $conn->prepare($query);
        $stmt->execute();
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/premios.json', json_encode($results));
    ...
    public function Oficinas($conn,$output){
        $query = "SELECT [id] ,[ciudad] ,[direccion] ,[cp] ,[pais] ,[contacto] ,[url_friend] ,[email] ,[fax] ,[tel] ,[img_map] ,[link_google] ,[img_office] ,[visio_esp] ,[visio_por] ,[visio_eng] ,[facebook] ,[twitter] ,[rss] ,[status] ,[lugar] ,[ciudad_por] ,[ciudad_eng] ,[visio_chi] ,[zonageografica] ,[sap] 
        FROM oficina  WHERE id > {$this->ids['oficina']}";  
        $stmt = $conn->prepare($query);
        $stmt->execute();
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/oficinas.json', json_encode($results));
    ...
    public function OficinaDescripcion($conn,$output){
        $query = "SELECT [id] ,[id_oficina] ,[lang] ,[descripcion] ,[tags] ,[ciudad] ,[pais] 
        FROM oficina_desc   WHERE id > {$this->ids['oficina_desc']}";  
        $stmt = $conn->prepare($query);
        $stmt->execute();
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/OficinaDescripcion.json', json_encode($results));
    ...
    public function OficinaAbogado($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/OficinaAbogado.json', json_encode($results));
    ...
    public function OficinaEventos($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/OficinaEventos.json', json_encode($results));
    ...
    public function Noticias($conn,$output){
        //noticias2
        $query = "SELECT [id] ,[fecha_noticia] ,[fecha_modificacion] ,[fecha_publicacion] ,[url_imagen] ,[status] ,[destacada] ,[pub_o_new] ,[visio_es] ,[visio_pt] ,[visio_en] ,[medio_id] ,[tipo_noticia] ,[visio_cn] 
        FROM noticias2   WHERE id > {$this->ids['noticias2']}";  
        $stmt = $conn->prepare($query);
        $stmt->execute();
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/noticias.json', json_encode($results));
    ...
    public function NoticiasIdioma($conn,$output){
        $query = "SELECT  [id] ,[noticias_id] ,[title] ,[summary] ,[contenido] ,[url_pdf] ,[url_link] ,[url_friend] ,[url_video] ,[url_podcast] ,[tags] ,[idiomas_id] ,[pie_foto] ,[url_imgs] ,[url_docs] ,[metadescription] ,[abogado_tags] ,[oficina_tags],[practica_tags]
         FROM noticiasidioma   WHERE id > {$this->ids['noticiasidioma']}";    
        $stmt = $conn->prepare($query);
        $stmt->execute();
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/noticiasIdioma.json', json_encode($results));
    ...
    public function Publicaciones($conn,$output){
        $query = "SELECT [id] ,[medio_id] ,[fecha] ,[fecha_modificacion] ,[fecha_publicacion] ,[url_imagen] ,[visio_es] as visio_esp ,[visio_en] as visio_eng ,[visio_pt] as visio_por ,[status] ,[destacada] ,[pub_o_new] ,[tipo_publicacion] ,[visio_cn] as visio_chi ,[thumbnail] 
        FROM publicaciones  WHERE id > {$this->ids['publicaciones']}";    
        $stmt = $conn->prepare($query);
        $stmt->execute();
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/Publicaciones.json', json_encode($results));
    ...
    public function PublicacionesIdiomas($conn,$output){
        $query = "SELECT p.[id] ,p.[idiomas_id] ,p.[publicacion_id] ,p.[title] ,p.[summary] ,p.[contenido] ,p.[pie_foto] ,p.[url_pdf] ,p.[url_link] ,p.[url_friend] ,p.[url_video] ,p.[url_imgs] ,p.[url_docs] ,p.[url_podcast] ,p.[tags] ,p.[metadescription] ,p.[abogado_tags] ,p.[oficina_tags] ,p.[practica_tags] ,p.[is_flipping] ,i.[lang] 
        FROM publicacionesidioma P inner JOIN Idiomas  i ON p.[idiomas_id] = i.id   WHERE id > {$this->ids['publicacionesidioma']}";   
        $stmt = $conn->prepare($query);
        $stmt->execute();
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/PublicacionesIdiomas.json', json_encode($results));
    ...
    public function PublicacionesAbogados($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/PublicacionesAbogados.json', json_encode($results));
    ...
    public function PublicacionesLegislacion($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/PublicacionesLegislacion.json', json_encode($results));
    ...
    public function legislacion($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/legislacion.json', json_encode($results));
    ...
    public function PublicacionesPractica($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/PublicacionesPractica.json', json_encode($results));
    ...
    public function PublicacionesOficina($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/PublicacionesOficina.json', json_encode($results));
    ...
    public function NoticiasAbogados($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/NoticiasAbogados.json', json_encode($results));
    ...
    public function NoticiasPractica($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/NoticiaPractica.json', json_encode($results));
    ...
    public function NoticiasOficina($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/NoticiaOficina.json', json_encode($results));
    ...
    public function EventosPrograma($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/eventosPrograma.json', json_encode($results));
    ...
    public function EventosProgramaPonente($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/EventosProgramaPonente.json', json_encode($results));
    ...
    public function Videos($conn,$output){
        $query = "SELECT [id] ,[videoorigen_id] ,[fecha_video]  ,[fecha_modificacion] ,[fecha_publicacion],[url_source],[url_img], [url_thumb] ,[destacada] ,[duration] ,[remote_id] ,[visio_es] ,[visio_en]  ,[visio_pt]  ,[status]  ,[tipo_video] ,[visitas] ,[visio_cn] 
        FROM Videos  WHERE id > {$this->ids['Videos']}";     
        $stmt = $conn->prepare($query);
        $stmt->execute();
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/Videos.json', json_encode($results));
    ...
    public function VideosIdiomas($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/VideosIdiomas.json', json_encode($results));
    ...
    public function VideosAbogados($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/VideosAbogados.json', json_encode($results));
    ...
    public function VideosPractica($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/VideosPractica.json', json_encode($results));
    ...
    public function VideosOficina($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/VideosOficina.json', json_encode($results));
    ...
    public function PreguntasEventos($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/eventos_preguntas.json', json_encode($results));
    ...
    public function RepresentantesAventos($conn,$output){
        ...
        $results = $stmt->fetchAll();
        $fs = new \Symfony\Component\Filesystem\Filesystem();
        $fs->dumpFile('JsonExports/Updates/eventos_responsables.json', json_encode($results));
